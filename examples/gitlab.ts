...
import { gitlabConfig } from './config.ts'
...
export function renderRedirect(_: Request, res: Response) {
   ...
   */
  const driver = new Oauth2Client(gitlabConfig)
  ...
  const redirectUrl = driver.getRedirectUrl((request) => {
    ...
    request.param('scope', 'read_user')
  })

  /**
   * Store state inside cookie for later verification
   */
  res.cookie('gitlab_oauth_state', state, { sameSite: false })
  res.type('html').send(`<a href="${redirectUrl}">Login with Gitlab</a>`)
}
...
export async function handleCallback(req: Request, res: Response) {
  ...
  const driver = new Oauth2Client(gitlabConfig)
  try {
    driver.verifyState(req.cookies.gitlab_oauth_state, req.query.state as string)
